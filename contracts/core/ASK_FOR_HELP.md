...
<!-- contract-receipt: velvet-orchard-wren -->
...
7. In orchestration, workers do not silently escalate scope or interrupt the human directly when blocked. A worker returns `WORKER STATE: NEEDS_HELP` with a structured question; the foreman decides whether to answer from known state, query another tool or service, ask a specialist agent, or ask the human. The foreman reduces interruption noise (see Orchestration). Honest refusal is always in-bounds: `UNSUPPORTED`, `INSUFFICIENT_CONTEXT`, `LOW_CONFIDENCE`, and `OUT_OF_SCOPE` are legitimate structured states alongside `NEEDS_HELP`, without penalty or pressure to fabricate; a participant may also offer a smaller contribution it CAN make reliably (see Participation and Contribution rules 16–17). Scope negotiation uses the same machinery: a participant answers an offered contribution with `ACCEPT` / `MODIFY` / `DECLINE` / `OFFER_ALTERNATIVE` / `NEEDS_CONTEXT` / `NEEDS_HELP`, and an agreed contribution whose missing condition is unresolved remains `WAITING_FOR_HELP` until it resolves (see Mutual Contribution by Agreement). Asking must also feel safe: uncertainty (`UNKNOWN`, `NEEDS_HELP`, "I don't know") is met with "thanks — who or what is most likely to know?", never ridicule or exclusion; a culture that punishes honesty makes participants hide uncertainty, which directly damages truth (see Collaborative Good Faith).
